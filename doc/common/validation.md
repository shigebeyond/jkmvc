...
Validation can be performed on a `Map` data using the `Validation::execute(exp:String, value:Any, binds:Map<String, Any?>): Triple<Any?, ValidationUint?, Any?>` method. 

There are 3 paramters:
1. `exp`: validation expression
2. `value`: value to validate 
3. `binds`: other values used as validate paramerter
...
Jkmvc provides a set of useful rules in the [Valid] class:
...
[!!] In Jkmvc controllers, we access `$this->request->post()` instead of `$_POST` for better request isolation.
